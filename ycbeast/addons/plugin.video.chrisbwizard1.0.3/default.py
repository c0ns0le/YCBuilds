...
##CREDIT TO LEE @ COMMUNITY BUILDS FOR WRITING THE FORCE CLOSE FUNCTION AND ANY OTHER DEVS WHO CREATED ANY MODULES USED CHRISB##

USER_AGENT = 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-GB; rv:1.9.0.3) Gecko/2008092417 Firefox/3.0.3'
base='http://chrisbwizard.pcriot.com'
ADDON=xbmcaddon.Addon(id='plugin.video.chrisbwizard')

    
    
VERSION = "1.0.3"
PATH = "ChrisB Wizard The best builds on kodi"            

    
def CATEGORIES():
    link = OPEN_URL('http://chrisbwizard.pcriot.com/wizard.txt').replace('\n','').replace('\r','')
    match = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)"').findall(link)
    addDir('Fresh Start','', 2, 'http://chrisbwizard.pcriot.com/pics/freshstart.png', 'http://chrisbwizard.pcriot.com/pics/freshstart.png','Perform Fresh Start')
...
def wizard(name,url,description):
    ...
    dp.create("ChrisB Wizard THE BEAST","Downloading ",'', 'Please Wait')
    ...
    time.sleep(2)
    dp.update(0,"", "Extracting Zip Please Wait THE BEAST WILL BE HERE SHORTLY")
    print '======================================='
    print addonfolder
    print '======================================='
    extract.all(lib,addonfolder,dp)
    dialog = xbmcgui.Dialog()
    dialog.ok("WELCOME TO THE BEAST", 'To ensure all changes are saved you must now close Kodi', 'to force close Kodi. Click ok,', 'DO NOT use the quit/exit options in Kodi.')
    killxbmc()

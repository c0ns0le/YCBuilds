...
packdir    =  xbmc.translatePath(os.path.join('special://home/addons/packages',''))
...
def WIZARDUPDATE():
    ...
    linkupdate = OPEN_URL('https://dg1tgb1503.fusa.be/wizard/schism/wizard_rel.txt').replace('\n','').replace('\r','')
    matchupdate = re.compile('name="(.+?)".+?rl="(.+?)".+?mg="(.+?)".+?anart="(.+?)".+?escription="(.+?)".+?ersion="(.+?)"').findall(linkupdate)
    ...
    for name,url,iconimage,fanart,description,version in matchupdate:
		sources.append(url)

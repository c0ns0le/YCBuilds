    ...
    try:
        ...
        for page in range( start, end):
                if ( page != start):
                        pageUrl = url + 'page/' + str(page) + '/'
                        html = net.http_GET(pageUrl).content                       
                match = re.compile('class="post-thumbnail"> <a\s*?href="(.+?)"> <img\s*?width="310" height="165" src="(.+?)" class="attachment-tie-medium wp-post-image" alt="(.+?)" />', re.DOTALL).findall(html)
                for movieUrl, img, name in match:
                        cm  = []
                        runstring = 'XBMC.Container.Update(plugin://plugin.video.allinone/?mode=Search11&query=%s)' %(name.strip())
        		cm.append(('[COLOR blue][B]E[/B][/COLOR]ntertainment [COLOR green]Search[/COLOR]', runstring))
...
def MovieMenu():   #movies
        ...
        #addon.add_directory({'mode': 'GetTitles83a', 'section': 'ALL', 'url': BASE_URL83 + '/category/movies/',
                             #'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR goldenrod](HD) [/COLOR][COLOR cornflowerblue][B]Latest Movies[/B][/COLOR] [COLOR green](tvmuseum) [/COLOR]>>'}, img=IconPath + 'tvmu.png', fanart=FanartPath + 'fanart.png')
        ...
        addon.add_directory({'mode': 'GetTitles66a', 'section': 'ALL', 'url': BASE_URL66 + '/movies-genre/',

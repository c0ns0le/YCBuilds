...
addon_id = 'plugin.video.ocw'
plugin = xbmcaddon.Addon(id=addon_id)
DB = os.path.join(xbmc.translatePath("special://database"), 'ocw.db')
BASE_URL = 'http://hotddl.co'
BASE_URL1 = 'http://watch8now.me/'
...
BASE_URL7 = 'http://upbuzz.net/'
net = Net()
addon = Addon('plugin.video.ocw', sys.argv)
...
def GetTitles10(text, img, section, url, startPage= '1', numOfPages= '1'): #tv
    try:
        pageUrl = url
        if int(startPage)> 1:
        ...
        for page in range( start, end):
                if ( page != start):
                        ...
                        html = net.http_GET(pageUrl).content
                match = re.compile('<div class="maintitle_base"><div class="maintitle_right"><h2 class="maintitle"><a href="(.+?)" rel="bookmark">(.+?)</a></h2></div></div><div class="post">', re.DOTALL).findall(html)
                for movieUrl, name in match:
                        addon.add_directory({'mode': 'GetLinks', 'section': section, 'url': movieUrl, 'img' : img, 'text' : name}, {'title':  name.replace('.', ' ')}, img=IconPath + 'newart.jpg', fanart=FanartPath + 'fanart.jpg') 
                addon.add_directory({'mode': 'GetTitles10', 'url': url, 'startPage': str(end), 'numOfPages': numOfPages}, {'title': '[COLOR blue][B][I]Next page...[/B][/I][/COLOR]'}, img=IconPath + 'nextpage.png', fanart=FanartPath + 'fanart.jpg')
    except:
...
def GetTitles10a(text, img, section, url, startPage= '1', numOfPages= '1'): #movies
    try:
        pageUrl = url
        if int(startPage)> 1:
                pageUrl = url + 'page/' + startPage + '/'
        ...
        for page in range( start, end):
                ...
                match = re.compile('<div class="maintitle_base"><div class="maintitle_right"><h2 class="maintitle"><a href="(.+?)" rel="bookmark">(.+?)</a></h2></div></div><div class="post">', re.DOTALL).findall(html)
                for movieUrl, name in match:
                        addon.add_directory({'mode': 'GetLinks', 'section': section, 'url': movieUrl, 'img' : IconPath + 'newart.jpg', 'text' : name}, {'title':  name.replace('.', ' ')}, img=IconPath + 'newart.jpg', fanart=FanartPath + 'fanart.jpg') 
                addon.add_directory({'mode': 'GetTitles10a', 'url': url, 'startPage': str(end), 'numOfPages': numOfPages}, {'title': '[COLOR blue][B][I]Next page...[/B][/I][/COLOR]'}, img=IconPath + 'nextpage.png', fanart=FanartPath + 'fanart.jpg')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]Sorry site mite be down [/B][/COLOR],[COLOR blue][B]Please try later[/B][/COLOR],7000,"")")
       	xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def GetTitles7(text, img, section, url, startPage= '1', numOfPages= '1'):
    try:
        ...
        for page in range( start, end):
                ...
                for movieUrl, name, img in match:
                        addon.add_directory({'mode': 'GetLinks', 'section': section, 'url': movieUrl, 'img' : img, 'text' : name}, {'title':  name.strip()}, img= img, fanart=FanartPath + 'fanart.jpg') 
                addon.add_directory({'mode': 'GetTitles7', 'url': url, 'startPage': str(end), 'numOfPages': numOfPages}, {'title': '[COLOR blue][B][I]Next page...[/B][/I][/COLOR]'}, img=IconPath + 'nextpage.png', fanart=FanartPath + 'fanart.jpg')
    except:
       	xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def GetTitles8(url):
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]Sorry site mite be down [/B][/COLOR],[COLOR blue][B]Please Come Back later[/B][/COLOR],7000,"")")
       	xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def GetTitles6a(text, img, query, section, name): 
    try:
        ...
        for img, name1, query1, name, sum, time in match:
                ...
                query = query.replace('The Royals ', 'The Royals 2015 ')
                query = query.replace("DC's Legends of Tomorrow", 'DCs Legends of Tomorrow')
                query = query.replace('Outsiders ', 'Outsiders 2016 ')
...
def GetTitles5a(text, img, query, url):
    try:
        pageUrl = url
        html = net.http_GET(pageUrl).content                     
        ...
        match2 = re.compile('<a href=".+?" class="previous-button inactive">Previous</a>\s*?<a href="(.+?)" class="next-button ">Next</a>',re.DOTALL).findall(html)
        ...
        for url in match2:
                addon.add_directory({'mode': 'GetTitles5a', 'url': 'http://www.moviefone.com' + url}, {'title': '[COLOR blue][B]Next Page...[/COLOR][/B]'}, img= 'https://raw.githubusercontent.com/MrEntertainment/backupREPO/master/plugin.video.allinone/icons/nextpage1.png', fanart= 'https://irenehyleung.files.wordpress.com/2012/12/2012-movie-collage31.jpg')
        setView('tvshows', 'tvshows-view')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]Sorry site is down [/B][/COLOR],[COLOR blue][B]Please Come Back later[/B][/COLOR],7000,"")")
       	xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def GetTitles3a(text, img, query, startPage= '1', numOfPages= '1'): 
    try:
        pageUrl = url
        if int(startPage)> 1:
                pageUrl = url + 'page/' + startPage + '/'
        ...
        for page in range( start, end):
                ...
                match = re.compile('<div class="thumbn"><img src="(.+?)" alt="(.+?)" /></div>\s*?<div class="filmkutu-bilgi-hover">\s*?<ul class="filmkutu-bilgi-hover-liste">\s*?<li class="izlenme">\s*?<a href=".+?">', re.DOTALL).findall(html)
                for img, query in match:
                        addon.add_directory({'mode': 'Search2', 'section': section, 'query': query, 'img' : img, 'text' : query}, {'title':  query}, img= img, fanart= 'http://www.parka-show.com/wp-content/uploads/2014/05/TV-Shows.jpg')
                addon.add_directory({'mode': 'GetTitles3a', 'url': url, 'startPage': str(end), 'numOfPages': numOfPages}, {'title': '[COLOR blue][B][I]Next page...[/B][/I][/COLOR]'}, img=IconPath + 'nextpage.png', fanart= 'http://www.parka-show.com/wp-content/uploads/2014/05/TV-Shows.jpg') 
        setView('tvshows', 'tvshows-view')   
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]Sorry site mite be down [/B][/COLOR],[COLOR blue][B]Please try later[/B][/COLOR],7000,"")")
       	xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def GetTitles2(section, query, url):
    try:
        html = net.http_GET(url).content
        listitem = GetMediaInfo(html)
        content = html
        match = re.compile('<img src="(.+?)" alt="(.+?)" >', re.DOTALL).findall(html)
        #match1 = re.compile('<a href="http://watch8now.me/tv/page/(.+?)">&gt;</a>', re.DOTALL).findall(html)
        for img, movieUrl in match:
                addon.add_directory({'mode': 'Search3', 'section': section, 'query': movieUrl}, {'title': movieUrl}, img= img, fanart= 'http://www.parka-show.com/wp-content/uploads/2014/05/TV-Shows.jpg') 
        #for url in match1:
                #addon.add_directory({'mode': 'GetTitles2', 'section': section, 'url': 'http://watch8now.me/tv/page/' + url}, {'title': '[COLOR blue][B][I]Next page...[/B][/I][/COLOR]'}, img=IconPath + 'nextpage.png', fanart=FanartPath + 'fanart.jpg') 
    except:
...
def Search1(text, img, section, query, name):
    try:
        url = 'http://hotddl.co' + query + '/'
        url = url.replace(' ', '-')
        print url
        html = net.http_GET(url).content
        listitem = GetMediaInfo(html)
        content = html
        match1 = re.compile('<p><img class="" src=".+?" alt="" width="250" height="355" /><br /> <strong>.+?</strong>.+?<br />(.+?)<br />', re.DOTALL).findall(html)
        match2 = re.compile('<a class="link" href="(http://uptobox.com/.+?)" target="_blank"').findall(content)
        match = re.compile('<a href=".+?" rel="nofollow" >(.+?)<', re.DOTALL).findall(html)
        #match3 = re.compile('<p><img class="" src="(.+?)" alt="" width=".+?" height=".+?" /><br />\s*?<strong>.+?</strong>.+?<br />\s*?(.+?)<br />').findall(content)
        match4 = re.compile('<strong>Source Used</strong>(.+?)<br />').findall(content)
        for name in match4:
                addon.add_directory({'listitem': listitem}, {'title': '[COLOR blue][B](' + name.replace('oneclickwatch.ws', 'mhd') + ')[/B][/COLOR]' }, img= img, fanart=FanartPath + 'fanart.jpg')
        for name in match1:
        ...
        #for img, name in match3:
                #addon.add_directory({'mode': 'PlayVideo1', 'url': url, 'listitem': listitem}, {'title': '[COLOR blue][B]' + name + '[/COLOR][/B]' }, img= img, fanart=FanartPath + 'fanart.jpg')
        for url in match2:
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No Links in tvm [/B][/COLOR],[COLOR blue][B]Trying Backup sites[/B][/COLOR],7000,"")")
    try:
        url = 'http://oneclickwatch.ws/000000/' + query
        url = url.replace(' ', '-')
        print url
        html = net.http_GET(url).content
        listitem = GetMediaInfo(html)
        content = html
        match = re.compile('<a href="(.+?)" rel="nofollow" target="_blank">.+?<', re.DOTALL).findall(html)
        match1 = re.compile('<h2 class="title">(.+?)</h2>', re.DOTALL).findall(html)
        match2 = re.compile('href="(http://uptobox.com/.+?)"').findall(content)
        listitem = GetMediaInfo(content)
        for name in match1:
                addon.add_directory({'listitem': listitem}, {'title': '[COLOR blue][B](' + name + ')[/B][/COLOR]' }, img= img, fanart=FanartPath + 'fanart.jpg')
        for url in match2:
                addon.add_directory({'mode': 'GetLinks1', 'url':  url, 'listitem': listitem , 'img': img, 'text': text }, {'title':  '[COLOR powderblue][B]UpToStream : direct link to[/B][/COLOR]' + ' - ' + text},img=img,  fanart=FanartPath + 'fanart.jpg')
        for url in match:
                host = GetDomain(url)
                if urlresolver.HostedMediaFile(url= url):
                        ...
                        title = title[2].replace('.html', '')
                        title = title.replace('.htm', '')
                        addon.add_directory({'mode': 'PlayVideo', 'url': url, 'listitem': listitem, 'img': img}, {'title':  '[COLOR lightcyan][B]' + host.replace('oneclickwatch.ws', 'TVDL') + '[/B][/COLOR]' + ' : ' + title + '[COLOR gold]  ...(HD-HQ) [tvdl][/COLOR]'}, img=img, fanart=FanartPath + 'fanart.jpg')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No Links in tvdl [/B][/COLOR],"")")
    try:
        url = 'http://myvideolinks.xyz/0000/00/00/' + query
        url = url.replace(' ', '-')
        print url
        html = net.http_GET(url).content
        listitem = GetMediaInfo(html)
        ...
        match1 = re.compile('<title>My Popcorn Time &raquo; (.+?)</title>', re.DOTALL).findall(html)
        listitem = GetMediaInfo(content)
        for name in match1:
                addon.add_directory({'listitem': listitem}, {'title': '[COLOR blue][B](' + name + ')[/B][/COLOR]' }, img= img, fanart=FanartPath + 'fanart.jpg')
        for url in match:
                ...
                if urlresolver.HostedMediaFile(url= url):
                        ...
                        title = title[2].replace('.html', '')
                        title = title.replace('.htm', '')
                        addon.add_directory({'mode': 'PlayVideo', 'url': url, 'listitem': listitem, 'img': img}, {'title':  '[COLOR lightcyan][B]' + host + '[/B][/COLOR]' + ' : ' + title + '[COLOR blue]  ...(HD-HQ) [mp][/COLOR]'}, img=img, fanart=FanartPath + 'fanart.jpg')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No Links in mp [/B][/COLOR],"")")
    try:
        url = 'http://upbuzz.net/' + query + '/'
        url = url.replace(' ', '-')
        print url
        html = net.http_GET(url).content
        listitem = GetMediaInfo(html)
        ...
        match1 = re.compile('<title>(.+?)</title>', re.DOTALL).findall(html)
        listitem = GetMediaInfo(content)
        for name in match1:
                addon.add_directory({'listitem': listitem}, {'title': '[COLOR blue][B](' + name + ')[/B][/COLOR]' }, img= img, fanart=FanartPath + 'fanart.jpg')
        for url in match:
                ...
                if urlresolver.HostedMediaFile(url= url):
                        ...
                        title = title[2].replace('.html', '')
                        title = title.replace('.htm', '')
                        addon.add_directory({'mode': 'PlayVideo', 'url': url, 'listitem': listitem, 'img': img}, {'title':  '[COLOR lightcyan][B]' + host + '[/B][/COLOR]' + ' : ' + title + '[COLOR rosybrown]  ...(HD-HQ) [buzz][/COLOR]'}, img=img, fanart=FanartPath + 'fanart.jpg')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No Links in upbuzz [/B][/COLOR],"")")
    try:
        ...
        for url in match:
                ...
                if urlresolver.HostedMediaFile(url= url):
                        ...
                        title = title[2].replace('.html', '')
                        title = title.replace('.htm', '')
                        addon.add_directory({'mode': 'PlayVideo', 'url': url, 'listitem': listitem, 'img': img}, {'title':  '[COLOR lightcyan][B]' + host + '[/B][/COLOR]' + ' : ' + title + '[COLOR violet]  ...(MQ-LQ) [WSO][/COLOR]'}, img=img, fanart=FanartPath + 'fanart.jpg')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No Links in WSO [/B][/COLOR],"")")
    try:
        url = 'http://onlinefreemovie.eu/' + query + '/'
        url = url.replace(' ', '-')
        print url
        html = net.http_GET(url).content
        listitem = GetMediaInfo(html)
        ...
        for url in match:
                ...
                if urlresolver.HostedMediaFile(url= url):
                        ...
                        title = title[2].replace('.html', '')
                        title = title.replace('.htm', '')
                        addon.add_directory({'mode': 'PlayVideo', 'url': url, 'listitem': listitem, 'img': img}, {'title':  '[COLOR lightcyan][B]' + host + '[/B][/COLOR]' + ' : ' + title + '[COLOR lime]  ...(MQ-LQ) [watchfree][/COLOR]'}, img=img, fanart=FanartPath + 'fanart.jpg')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No Links in OFM [/B][/COLOR],[COLOR blue][B]Trying Backup site[/B][/COLOR],2000,"")")
    try:
    ...
    try:
        ...
        for url in match:
                ...
                if urlresolver.HostedMediaFile(url= url):
                        ...
                        title = title[2].replace('.html', '')
                        title = title.replace('.htm', '')
                        addon.add_directory({'mode': 'PlayVideo', 'url': url, 'listitem': listitem, 'img': img}, {'title':  '[COLOR lightcyan][B]' + host + '[/B][/COLOR]' + ' : ' + title + '[COLOR violet]  ...(HD-HQ) [buzz][/COLOR]'}, img=img, fanart=FanartPath + 'fanart.jpg')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No Links in upbuzz [/B][/COLOR],"")")
    try:
        url = 'http://www.perfecthdmovies.com/hollywood/' + query
        url = url.replace(' ', '-')
        print url
        html = net.http_GET(url).content
        listitem = GetMediaInfo(html)
        ...
        for url in match:
                ...
                if urlresolver.HostedMediaFile(url= url):
                        ...
                        title = title[2].replace('.html', '')
                        title = title.replace('.htm', '')
                        addon.add_directory({'mode': 'PlayVideo', 'url': url, 'listitem': listitem, 'img': img}, {'title':  '[COLOR lightcyan][B]' + host + '[/B][/COLOR]' + ' : ' + title + '[COLOR lightgreen]  ...(HD-HQ) [pm][/COLOR]'}, img=img, fanart=FanartPath + 'fanart.jpg')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No Links in pm [/B][/COLOR],"")")
    try:
        url = 'http://oneclickwatch.ws/000000/' + query
        url = url.replace(' ', '-')
        print url
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No Links in tvdl [/B][/COLOR],"")")
    try:
        url = 'http://watchitvideos.com/' + query + '-putlocker2/'
        url = url.replace(' ', '-')
        print url
        html = net.http_GET(url).content
        listitem = GetMediaInfo(html)
        content = html
        match = re.compile('title=".+?" rel="nofollow" target="_blank" href="(.+?)"><img', re.DOTALL).findall(html)
        match1 = re.compile('rel="profile" href=".+?" /><title>(.+?)</title><meta', re.DOTALL).findall(html)
        match2 = re.compile('href="(http://uptobox.com/.+?)"').findall(content)
        listitem = GetMediaInfo(content)
        for name in match1:
                addon.add_directory({'listitem': listitem}, {'title': '[COLOR blue][B](' + name + ')[/B][/COLOR]' }, img= img, fanart=FanartPath + 'fanart.jpg')
        ...
        for url in match:
                ...
                if urlresolver.HostedMediaFile(url= url):
                        ...
                        title = title[2].replace('.html', '')
                        title = title.replace('.htm', '')
                        addon.add_directory({'mode': 'PlayVideo', 'url': url, 'listitem': listitem, 'img': img}, {'title':  '[COLOR lightcyan][B]' + host + '[/B][/COLOR]' + ' : ' + title + '[COLOR orangered]  ...(HD-HQ) [WIV][/COLOR]'}, img=img, fanart=FanartPath + 'fanart.jpg')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No Links in WIV [/B][/COLOR],"")")
    try:
        ...
        for url, title, img in match:
                addon.add_directory({'mode': 'GetLinks', 'url': url, 'img': img, 'text': text }, {'title':  title + ' [COLOR red]...(300mbmovies4u)[/COLOR]'}, img= img, fanart=FanartPath + 'fanart.png')
    except:
...
def Search3(img, text, query):
    try:
        ...
        match2 = re.compile('<link rel="next" href="(.+?)"', re.DOTALL).findall(html)
        ...
        for url in match2:
                addon.add_directory({'mode': 'GetTitles7', 'url': url}, {'title':  '[COLOR blue][B][I]Next page...(tvdl)[/B][/I][/COLOR]'}, img=IconPath + 'nextpage.png', fanart=FanartPath + 'fanart.jpg')
        #for title in match1:
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]Checking mhd FOR LINKS[/B][/COLOR],[COLOR blue][B]Please try later[/B][/COLOR],7000,"")")
    try:
        ...
        for url, img, title in match:
                addon.add_directory({'mode': 'GetLinks', 'url': url, 'img': img, 'text': title }, {'title':  title + '   ...(MP)...' }, img= img, fanart=FanartPath + 'fanart.jpg')
        setView('tvshows', 'calendar-view')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]Checking upbuzz FOR LINKS[/B][/COLOR],[COLOR blue][B]Please try later[/B][/COLOR],7000,"")")
    try:
        ...
        match = re.compile('<h2 class="title"><a href="(.+?)" title="Permalink.+?">(.+?)</a></h2>.+? src="(.+?)"', re.DOTALL).findall(html)
        for url, title, img in match:
                addon.add_directory({'mode': 'GetLinks', 'url': url, 'img': img, 'text': title }, {'title':  title + '   ...(buzzup)...' }, img= img, fanart=FanartPath + 'fanart.jpg')
        setView('tvshows', 'calendar-view')
    except:
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No LINKS in upbuzz[/B][/COLOR],[COLOR blue][B]Please try later[/B][/COLOR],7000,"")")
       	xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def GetLinks(text, img, section, url):
    try:
        ...
        match3 = re.compile('href=".+?">(.+?)</a><br />').findall(content)
        listitem = GetMediaInfo(content)
        for url in match1:
...
def GetLinks1a(text, img, section, url):
        ...
        for url, name, name1 in match:
                addon.add_directory({'mode': 'PlayVideo1', 'url': 'http:' + url, 'listitem': listitem, 'img': img, 'text': text }, {'title': '[COLOR blue][B]' + name + ' : ' + name1 + '[/B][/COLOR]' + ' - ' + text }, img=img, fanart=FanartPath + 'fanart.jpg')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def MainMenu():    #homescreen
        addon.add_directory({'mode': 'menu2'}, {'title': '[COLOR blue][B]TvMuseum [/B] [/COLOR]>>'}, img=IconPath + 'home.png', fanart=FanartPath + 'fanart.jpg')

        addon.add_directory({'mode': 'menu3'}, {'title': '[COLOR blue][B]BuzzUp [/B] [/COLOR]>>'}, img=IconPath + 'home.png', fanart=FanartPath + 'fanart.jpg')
        ...
        addon.add_directory({'mode': 'GetTitles2', 'section': 'ALL', 'url': BASE_URL1 + '/tv-series'}, {'title':  '[COLOR greenyellow][B]TV Index Search[/B] (TV Episodes)[/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        ...
        addon.add_directory({'mode': 'menu1'}, {'title': '[COLOR springgreen][B]Movies Index Search[/B] (Genres)[/COLOR] '}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg') 
        #addon.add_directory({'mode': 'GetTitles5a', 'url': BASE_URL5 + '/dvd/'}, {'title':  '[COLOR springgreen][B]Movies Index Search[/B] (DVD)[/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles5a', 'url': BASE_URL5 + '/new-movie-releases/'}, {'title':  '[COLOR springgreen][B]Movies Index Search[/B] (Box Office)[/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        ...
        addon.add_directory({'mode': 'GetSearchQuery7'},  {'title':  '[COLOR green][B]OCWReborn Search[/B][/COLOR]'}, img=IconPath + 'searchs1.png', fanart=FanartPath + 'fanart.jpg')
        ...
        #addon.add_directory({'mode': 'ResolverSettings'}, {'title':  '[B][COLOR red]WARNING !! DONT TRUST THIS [/B][/COLOR] [COLOR green]Umpa Lumpa [/COLOR]'}, img= 'http://s13.postimg.org/sxbr52won/faceinhole.jpg', fanart=FanartPath + 'newart.jpg')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))

def Menu2():
        addon.add_directory({'mode': 'GetTitles10', 'section': 'ALL', 'url': BASE_URL + '/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]TVM : Home Page[/B] [/COLOR]>>'}, img=IconPath + 'home.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles10', 'section': 'ALL', 'url': BASE_URL + '/movies/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]TVM : Latest Added Movies[/B] [/COLOR]>>'}, img=IconPath + 'movies1.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles10a', 'section': 'ALL', 'url': BASE_URL + '/tv-shows/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]TVM : Latest Added Episodes[/B] [/COLOR]>>'}, img=IconPath + 'tv2.png', fanart=FanartPath + 'fanart.jpg')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))

def Menu3():
        addon.add_directory({'mode': 'GetTitles7', 'section': 'ALL', 'url': BASE_URL7 + '/',
                             'startPage': '1', 'numOfPages': '2'}, {'title':  '[COLOR blue][B]BuzzUp : Home Page[/B] [/COLOR]>>'}, img=IconPath + 'tv2.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles7', 'section': 'ALL', 'url': BASE_URL7 + '/tag/tv/',
                             'startPage': '1', 'numOfPages': '2'}, {'title':  '[COLOR blue][B]BuzzUp : Latest Added Episodes[/B] [/COLOR]>>'}, img=IconPath + 'tv2.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles7', 'section': 'ALL', 'url': BASE_URL7 + '/tag/movies/',
                             'startPage': '1', 'numOfPages': '2'}, {'title':  '[COLOR blue][B]BuzzUp : Latest Added Movies[/B] [/COLOR]>>'}, img=IconPath + 'tv2.png', fanart=FanartPath + 'fanart.jpg')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))

#------------------------------------------------------------------------------------ Movie Genres search --------------------------------------------------------------------------------------------#

def Menu1():
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Latest[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/action/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Action[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/adventure/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Adventure[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/animation/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Animation[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/comedy/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Comedy[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/crime/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Crime[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/drama/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Drama[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/family/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Family[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/fantasy/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Fantasy[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/horror/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Horror[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/mystery/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Mystery[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/romance/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Romance[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/sci-fi/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Sci-fi[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        addon.add_directory({'mode': 'GetTitles3a', 'section': 'ALL', 'url': BASE_URL2 + '/category/movies/thriller/',
                             'startPage': '1', 'numOfPages': '1'}, {'title':  '[COLOR blue][B]Thriller[/B] [/COLOR]'}, img=IconPath + 'indexs.png', fanart=FanartPath + 'fanart.jpg')
        xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
def GetSearchQuery():
	...
	if (keyboard.isConfirmed()):
                ...
                return  
def Search(query):
        url = 'http://www.google.com/search?q=site:tvmuseum.freexblogs.com/ ' + query
        url = url.replace(' ', '+')
        print url
        html = net.http_GET(url).content
...
def Search7(query):
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification([COLOR red][B]No LINKS in upbuzz[/B][/COLOR],[COLOR blue][B]Please try later[/B][/COLOR],7000,"")")
    try:
        url = 'http://hotddl.co/?s=' + query
        url = url.replace(' ', '+')
        print url
        html = net.http_GET(url).content
        match = re.compile('<div class="maintitle_base"><div class="maintitle_right"><h2 class="maintitle"><a href="(.+?)" rel="bookmark">(.+?)</a></h2></div></div>', re.DOTALL).findall(html)
        for url, title in match:
                addon.add_directory({'mode': 'GetLinks2', 'url': url}, {'title':  title + '   ...(tvm)...'  }, img= 'http://4.bp.blogspot.com/-p0xJTyt30yE/TWOcirGIzvI/AAAAAAAAAxE/hD2EethE8-U/s400/television_icon_265115133_std.png', fanart=FanartPath + 'fanart.jpg')
        setView('tvshows', 'calendar-view')
    except:
...
def GetLinks2(section, url):
        ...
        match = re.compile('href="(.+?)"').findall(content)
        match1 = re.compile('href="(http://uptobox.com/.+?)"').findall(content)
        match2 = re.compile('<a href=.+?rel=nofollow>(.+?)</a><br />').findall(content)
        listitem = GetMediaInfo(content)
        for url in match1:
...
def GetLinks2b(section, url):
        ...
        listitem = GetMediaInfo(content)
        for url, name, name1 in match:
                addon.add_directory({'mode': 'PlayVideo2', 'url': url, 'listitem': listitem}, {'title': name + ' : ' + name1 }, img= 'https://uptostream.com/images/logo.png', fanart=FanartPath + 'fanart.jpg')
       	xbmcplugin.endOfDirectory(int(sys.argv[1]))
...
if mode == 'menu3':
	...
	GetTitles7(text, img, section, url, startPage, numOfPages)
elif mode == 'GetTitles2': 
	GetTitles2(section, query, url)
elif mode == 'GetTitles3': 
	GetTitles3(text, img, query, startPage, numOfPages)
elif mode == 'GetTitles3a': 
	GetTitles3a(text, img, query, startPage, numOfPages)
elif mode == 'GetTitles4': 
	GetTitles4(text, img, query)
elif mode == 'GetTitles5a': 
	GetTitles5a(text, img, query, url)
	...
	GetTitles10(text, img, section, url, startPage, numOfPages)
elif mode == 'GetTitles10a': 
	GetTitles10a(text, img, section, url, startPage, numOfPages)
elif mode == 'GetLinks':

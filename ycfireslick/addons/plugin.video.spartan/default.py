...
ADDON_NAME   = xbmcaddon.Addon('plugin.video.spartan')

BASE_URL     = 'http://spartanpixel.net/'
WIZARD_PAGE  = 'wizard.txt'
	...
	try:
		for item in __in.infolist():
			...
			if progress.iscanceled():
				...
				dialog.ok('Spartan Wizard', 'Extraction was cancelled.')
...
def hook(numblocks, blocksize, filesize, url, progress):
	...
	if progress.iscanceled():
		dialog = xbmcgui.Dialog()
		dialog.ok('Spartan Wizard', 'Downloading was cancelled.')
...
def runAddon(name, url, desc):
	...
	progress.create('Spartan Wizard', 'Downloading...')
...
def killXbmc():
	dialog = xbmcgui.Dialog()
	dialog.ok('Spartan Wizard', 'Amazon and Android, please pull the power cable from your device.')
	
	choice = xbmcgui.Dialog().yesno('Spartan Wizard', 'You are about to force close. Would you like to continue?', yeslabel='OK', nolabel='Cancel')
	...
	if (platform == 'OSX'):
		...
		dialog.ok('Spartan Wizard', 'Please pull the power cable from your device.')

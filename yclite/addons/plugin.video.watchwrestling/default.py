...
def GetLinks(url):
        ...
        if re.search('"lockerId":', url_content):
            ...
            for hidden_item in re.finditer("(?s)<p>.+?<div.+?><span.+?>(.+?)</.+?<p>(.+?)</p>",hidden_items):
                ...
                for hi_link in re.finditer("(?s)<a.+?href=\"(.+?)\".+?>(.+?)</", hi_links):
                    ...
                    contextMenuItems = []
                    queries = {'mode' : 'GetMedia', 'url' : hi_link_url+'|'+REF, 'title' : title + super_header_title + ' - ' + hi_link_title, 'historytitle' : title, 'historylink': sys.argv[0]+sys.argv[2], 'img':img}
    ...
    for regular_item in re.finditer(regular_item_re,url_content):
        ...
        for ri_link in re.finditer("(?s)<a.+?href=\"(.+?)\".+?>(.+?)</", ri_links):
            ...
            contextMenuItems = []
            queries = {'mode' : 'GetMedia', 'url' : ri_link_url+'|'+REF, 'title' : title + super_header_title + ' - ' + ri_link_title, 'historytitle' : title, 'historylink': sys.argv[0]+sys.argv[2], 'img':img}
...
def GetMedia(url):
    ...
    if 'protect.cgi' in url:
        headers={'Referer':ref,'Host': 'wrestlingreviews.net','User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/48.0.2564.116 Safari/537.36'}
        link=net.http_GET(url, headers=headers).content
        link=link.replace('<IFRAME','<iframe').replace('SRC="','src="')

...
class myvideolinks(MovieSource, TVShowSource):
    ...
    def GetFileHosts(self, url, list, lock, message_queue):
        ...
        net = Net(user_agent='Mozilla/5.0 (Windows NT 6.1; WOW64; Trident/7.0; rv:11.0) like Gecko')
    ...
    def GetFileHostsForContent(self, title, name, year, season, episode, type, list, lock, message_queue):
        ...
        tv_url='http://myvideolinks.xyz/?s=%s+S%sE%s' %(name.replace(' ','+'),season_pull,episode_pull)
        movie_url='http://myvideolinks.xyz/?s=%s+%s' %(name.replace(' ','+'),year)

...
class movie25(MovieIndexer, MovieSource):
    ...
    def ExtractContentAndAddtoList(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''): 
        ...
        if section == 'featured' or 'new-release' or 'latest-added' or 'latest-hd' or 'most-popular' or 'most-voted' or 'az' or 'genres':
            match=re.compile('<div class="movie_pic"><a href="(.+?)"  target="_self" title="Watch (.+?) \((.+?)\)">').findall(html)
    ...
    def GetFileHosts(self, url, list, lock, message_queue):
        ...
        for p in link:
            p=p.replace('\n','').replace('\t','')
            try:
                match=re.compile('<li id="link_name">(.+?)</li><li id="playing_button"><a href="(.+?)" target=').findall(p)
    ...
    def GetFileHostsForContent(self, title, name, year, season, episode, type, list, lock, message_queue):
        ...
        import urllib
        movie_url = self.base_url + ( 'search.php?key=%s' % urllib.quote(name + ' ' + year ) )
        print movie_url
        print '#################################################################################################'
        content = net.http_GET(movie_url).content
        
        search_result = re.search(r'<div class="movie_pic"><a href="(.+?)"  target="_self" title=".+?">', content)
        if search_result:
    ...
    def Resolve(self, url):
        ...
        match2=re.compile('<div><IFRAME id="showvideo" src="(.+?)" scrolling').findall(html)
        for url in match2:
            url = url.replace('http://movie25.hk/tz.php?url=','')

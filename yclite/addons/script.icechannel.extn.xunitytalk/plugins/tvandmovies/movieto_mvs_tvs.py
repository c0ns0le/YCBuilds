...
class movieto(MovieSource,TVShowSource):
    ...
    def GetFileHostsForContent(self, title, name, year, season, episode, type, list, lock, message_queue):
        ...
        headers={'User-Agent':'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/47.0.2526.111 Safari/537.36',
                 'Cookie': 'announcement=%s; __cfduid=%s; PHPSESSID=%s; notice=true; HelloWorld=1; HelloWorld1=1;' % (TIME,cfduid,token),
                 'Referer':'http://movietv.to',
        ...
        for URL , TITLE in match:
                ...
                if name.lower() in TITLE.lower():
                    if type == 'tv_episodes':
                        ...
                        LINKURL='http://movietv.to/series/getLink?id=%s&s=%s&e=%s&token=%s&_=%s' % (id,season,episode,token,TIME)

...
def TV():
        ...
        addDirTV('Most Popular',custurltv1,21,os.path.join(art,'MOST_POPULAR.png'),None)
        addDirTV('Newest Episodes',custurltv1+'latest',36,os.path.join(art,'NEW_EPISODES.png'),None)
        addDirTV('A-Z',custurltv1+'letters/A',33,os.path.join(art,'A_Z.png'),None)
        addDirTV('Genres',custurltv1+'genres/drama',35,os.path.join(art,'GENRE.png'),None)
        addDirTV('TV Schedule',custurltv1+'tvschedule/-1',38,os.path.join(art,'TV_SCHEDULE.png'),None)
        addDirTV('Search',custurltv1,37,os.path.join(art,'search1.png'),None)
        addDirTV('Favorites',custurltv1,39,os.path.join(art,'FAVORITE.png'),None)
        if MYFAVS:
                addDir('Kid TV Shows',custurl1+'',45,os.path.join(art,'KIDS_TV.png'),None,None)
...
#Popular TV menu
def POPULAR():
        addDirTV('Popular this week',custurltv1+'new',22,os.path.join(art,'MOST_POPULAR.png'),None)
        addDirTV('Popular Series',custurltv1,26,os.path.join(art,'MOST_POPULAR.png'),None)
        addDirTV('Popular Cartoons',custurltv1,27,os.path.join(art,'MOST_POPULAR.png'),None)
        addDirTV('Popular Documentaries',custurltv1,28,os.path.join(art,'MOST_POPULAR.png'),None)
        addDirTV('Popular Shows',custurltv1,29,os.path.join(art,'MOST_POPULAR.png'),None)
        addDirTV('Popular Sports',custurltv1,30,os.path.join(art,'MOST_POPULAR.png'),None)
...
def SCHEDULE(url):
        ...
        for url,name in match:
                ...
                if ok in url:
                        if name not in nono:
                                addDirTV(name,custurltv+url,36,'',None)
...
def SEASONS(name,url):
        ...
        for url,name in match:
                addDirTV(name,custurltv+url+'@'+title,32,'','seasons')
...
def VIDEOLINKSTV(url):
        url2 = url.split('/')[4].replace('.html','')

        episode = url2.split('_')[2]
        season = url2.split('_')[1]
        title = url2.split('_')[0]
...
def AZ(url):
        ...
        for url,name in match2:
                ok = ['0-9']
                if name in ok:
                        addDir(name,custurl+url,1,'',None,None)
        for url,name in match:
                ...
                if name in ok:
                        ...
                        if name in A:
                                url = '/movies/a/'
                        addDir(name,custurl+url,1,icon,None,None)
